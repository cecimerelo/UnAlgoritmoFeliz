...
function evolution(population_in_castes, population_model)
    ...
    new_alpha_individuals = [crossover_operator(alpha_parents,population_model.config_parameters) for alpha_parents in alpha_reproduction_pool]    
    new_beta_individuals = [crossover_operator(alpha_beta_parents, population_model.config_parameters) for alpha_beta_parents in beta_reproduction_pool]
end
